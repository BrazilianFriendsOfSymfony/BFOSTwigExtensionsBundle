<?php
namespace BFOS\TwigExtensionsBundle\Form\ChoiceList;

use Doctrine\ORM\EntityManager;
use Symfony\Component\Form\Util\PropertyPath;

class EntityAjaxChoiceList
{
    protected $choices;
     ...
     */
    public function __construct(EntityManager $em, $class, $property = null, $queryBuilder = null)
    {
...
//        $this->choices = $choices;
     ...
     */
    protected function load($ids = array())
    {
        ...
        if (is_array($this->choices)) {
            ...
            if(count($qb->getDQLPart('where'))>0){
                $qb->andWhere($qb->expr()->in("$rootAlias.id ", $ids));
            } else {
                $qb->where($qb->expr()->in("$rootAlias.id ", $ids));
            }
            $entities = $qb->getQuery()->execute();
        } else {
            $entities = $this->em->getRepository($this->class)->findBy(array('id' => $ids));
        }
    ...
    private function loadEntities($entities, $group = null)
    {
        ...
        foreach ($entities as $key => $entity) {
            ...
            if (count($this->identifier) > 1) {
                ...
                $id = $key;
                // TODO: throw an Exception - multi id is not supported
            } else {
     ...
     */
    public function getEntitiesByIds($ids = array())
    {
        if (!$this->loaded) {
            $this->load($ids);
        }
